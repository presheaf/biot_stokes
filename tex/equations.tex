...
Multiply \eqref{eq:stokes_stress} by \vf and integrate over \stokes. By standard vector calculus,
...
\item I derived the weak formulation by noting that \cite{ambartsumyan} had $\mult = -(\sigmabf \nf) \cdot \nf = \pp$, while I want $\mult = -(\sigmabf \nf) \cdot \nf = \pp + C_p$, so I figured I could get my weak formulation by taking \cite{ambartsumyan}'s and replacing $\mult$ by $\mult + C_p$. I should rederive it myself to see whether this is actually true.
